...
const CapturePage = () => {
  ...
  const sendFileToBackend = async (file) => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:5000/api/results", formData, {
    ...
    <>
      ...
      {result && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="mt-8 grid sm:grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-4xl"
        >
          {result.finalDetections.map((d, i) => (
            <motion.div
              key={i}
              whileHover={{ scale: 1.03 }}
              className="bg-white/20 backdrop-blur-md p-4 rounded-2xl shadow-lg text-white"
            >
              <h3 className="text-lg font-semibold">{d.species}</h3>
              <p className="mt-1 text-indigo-300 font-medium">Count: {d.count}</p>
            </motion.div>
          ))}
        </motion.div>
      )}
